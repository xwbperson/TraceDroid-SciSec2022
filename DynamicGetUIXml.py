...
# 用于存放 XML 文件分析的结果，包括节点树和节点的深度信息
xmlHashDict = {}  # 存放xml的tree:level的值


def processAppToGetUIXml(device_id):
    index = 0
    # 获取指定设备上当前正在运行的应用的包名和活动名称，以及当前界面的xml布局树信息
    ...
    # 打印当前界面对应的app的包名和启动的activity（最好是开始就进入mainActivity）
    appPackage = d.current_app()['package']
    fileDir = filePath + "/" + appPackage
    # 辅助文件，用来每次存取当前遍历界面的布局树
    helpXmlName = fileDir + "/" + 'help.xml'
    ...
    获取当前activity中可点击的坐标列表，并将其存入click_coord_list中
    """
...
def writeToTxt(xml, fileName):
    # startStr = '<node index="0" text="" resource-id="" class="android.widget.FrameLayout"
    # package="com.android.systemui"' endStr = '<node index="3" text=""
    # resource-id="com.android.systemui:id/scrim_in_front" class="android.view.View" package="com.android.systemui"
    ...
    # visible-to-user="true" bounds="[0,0][1080,72]" />\r\n  </node>\r\n'
    #
